...
A stripped-down variant without the bottom control bar lives at [prototype-environment-clean](https://github.com/madhumadhupria/prototype-environment-clean).
...
4. **Before the model loads**, the viewer requests an access token from the token URL (currently Vercel).
5. **With that token**, the viewer opens the model from APS.
6. **The custom extension** applies the CAD/BIM neutral backdrop and grid. The bottom prototype strip lets you switch environments, section tools, rendering detail, and rotate mode.
...
extension/               # CAD/BIM backdrop, grid, prototype strip, section tools
